...
OperatSDI <-
  function(lon,
    ...
    if (is.na(as.Date(start.date, "%Y-%m-%d")) == TRUE ||
        is.na(as.Date(end.date, "%Y-%m-%d")) == TRUE ||
        TS < 1 ||
        TS > 96 || all.equal(TS, as.integer(TS)) != TRUE) {
      stop(
    ...
    if (mim.date.fit < 7) {
      ...
      if (PEMethod == "HS") {
        ...
        Ra <-
          (37.6 * (dist.terra.sol ^ 2)) * ((pi / 180) * hn.deg * sin(lat.rad) * sin(decli.rad) +
                                             (cos(lat.rad) * cos(decli.rad) * sin(hn.rad)))
        ETP.harg.daily <-
          0.0023 * (Ra * 0.4081633) * (sse_i$T2M_MAX - sse_i$T2M_MIN) ^ 0.5 * (sse_i$T2M + 17.8)
      ...
      if (PEMethod == "PM") {
        ...
        Q0.ajust <- 0.75 * Ra
        Rn <-
          (1 - 0.2) * sse_i$ALLSKY_SFC_SW_DWN - (1.35 * (sse_i$ALLSKY_SFC_SW_DWN / Q0.ajust) - 0.35) * (0.35 - (0.14 * sqrt(ea))) * (5.67 * 10 ^
                                                                                                                                       -8) * (((sse_i$T2M ^ 4) + (sse_i$T2M_MIN ^ 4)) / 2)
        ETP.pm.daily <-
          (0.408 * slope.pressure * (Rn - 0.8) + 0.063 * (900 / (sse_i$T2M + 273)) * sse_i$WS2M * (es - ea)) / (slope.pressure + 0.063 * (1 + 0.34 * sse_i$WS2M))
      ...
      if (TS > 1) {
        ...
        c <- 1
        data.at.timescale[c, ] <-
          c(data.week[b, 1:4], data.week[b, 8], colSums(data.week[a:b, 6:7]))
        point <- point + 1
        a <- a + 1
        b <- b + 1
        c <- c + 1
        ...
        data.at.timescale[, ] <-
          as.matrix(c(data.week[, 1:4], data.week[, 8], data.week[, 6:7]))
      }
      data.at.timescale <-
        as.matrix(cbind(
          data.at.timescale,
      ...
      parameters <-
        as.data.frame(parms[which(parms[, 1] == lon &
                                    parms[, 2] == lat &
                                    parms[, 13] == TS), ])
      if (length(parameters[, 1]) == 0) {
        ...
        if (distr == "GEV") {
          for (pos in 1:n.weeks) {
            ...
            prob <-
              (par[6] + (1 - par[6])) * cdfgam(data.at.timescale[pos, 6], c(par[4], par[5]))
            ...
            if (PEMethod == "PM") {
              prob <-
                cdfgev(data.at.timescale[pos, 8], c(par[10], par[11], par[12]))
            }
            if (is.na(prob) == FALSE & prob < 0.001351) {
              prob <- 0.001351
            }
        ...
        if (distr == "GLO") {
          for (pos in 1:n.weeks) {
            ...
            if (PEMethod == "PM") {
              prob <-
                cdfglo(data.at.timescale[pos, 8], c(par[10], par[11], par[12]))
            }
            if (is.na(prob) == FALSE & prob < 0.001351) {
              prob <- 0.001351
            }
    ...
    if (final.month == 2) {
      if (end.user.day == 7 || end.user.day == 14 ||
          ...
          end.user.day == 28 || end.user.day == 29) {
        message("Done")
      } else {
        message("The latest quart.month period is not complete.")
      }
