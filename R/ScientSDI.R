...
#' @examples
#' ScientSDI(lon=-47.3, lat=-22.87, start.date="1991-01-01", end.date="2022-12-31",TS=4,Good="no",sig.level=0.95,distr="GEV",RainUplim=NULL,RainLowlim=NULL,PEUplim=NULL,PELowlim=NULL)
...
ScientSDI=function(lon,lat,start.date, end.date,distr="GEV",TS=4,Good="Yes",sig.level=0.95,
  ...
  if(distr=="GEV" || distr=="GLO"){
    if(distr=="GEV"){distribution=1}
    if(distr=="GLO"){distribution=2}
    if (Good=="Yes" || Good=="YES" || Good=="YeS" || Good=="YEs"|| Good=="yes" ||
      ...
      if(is.na(as.Date(end.date, "%Y-%m-%d"))==TRUE || is.na(as.Date(start.date, "%Y-%m-%d"))==TRUE
        ...
        message("Recall Date format should be YYYY-MM-DD and TS must be an interger  value ranging between 1 and 96")} else {
...
Why don't you select a longer period between start.date and end.date?")}
            ...
            if (start.user.day>14 & start.user.day<=21){dif=start.user.day-15;start.week=3}
            if (start.user.day>=22){dif=start.user.day-22;start.week=4}
            ...
            final.year=sse_i$YEAR[n.tot];initial.year=sse_i$YEAR[1]
            ...
            n.years=1+(final.year-initial.year);total.nweeks=48*n.years
            a=1;b=2;c=3;d=4;
            ...
            for (i in 1:n){
              if(data.week[i,4]==1 & data.week[i,5]==1){quart.month[i,1]=1}
              if(data.week[i,4]==1 & data.week[i,5]==2){quart.month[i,1]=2}
              if(data.week[i,4]==1 & data.week[i,5]==3){quart.month[i,1]=3}
              if(data.week[i,4]==1 & data.week[i,5]==4){quart.month[i,1]=4}
              if(data.week[i,4]==2 & data.week[i,5]==1){quart.month[i,1]=5}
              if(data.week[i,4]==2 & data.week[i,5]==2){quart.month[i,1]=6}
              if(data.week[i,4]==2 & data.week[i,5]==3){quart.month[i,1]=7}
              if(data.week[i,4]==2 & data.week[i,5]==4){quart.month[i,1]=8}
              if(data.week[i,4]==3 & data.week[i,5]==1){quart.month[i,1]=9}
              if(data.week[i,4]==3 & data.week[i,5]==2){quart.month[i,1]=10}
              if(data.week[i,4]==3 & data.week[i,5]==3){quart.month[i,1]=11}
              if(data.week[i,4]==3 & data.week[i,5]==4){quart.month[i,1]=12}
              if(data.week[i,4]==4 & data.week[i,5]==1){quart.month[i,1]=13}
              if(data.week[i,4]==4 & data.week[i,5]==2){quart.month[i,1]=14}
              if(data.week[i,4]==4 & data.week[i,5]==3){quart.month[i,1]=15}
              if(data.week[i,4]==4 & data.week[i,5]==4){quart.month[i,1]=16}
              if(data.week[i,4]==5 & data.week[i,5]==1){quart.month[i,1]=17}
              if(data.week[i,4]==5 & data.week[i,5]==2){quart.month[i,1]=18}
              if(data.week[i,4]==5 & data.week[i,5]==3){quart.month[i,1]=19}
              if(data.week[i,4]==5 & data.week[i,5]==4){quart.month[i,1]=20}
              if(data.week[i,4]==6 & data.week[i,5]==1){quart.month[i,1]=21}
              if(data.week[i,4]==6 & data.week[i,5]==2){quart.month[i,1]=22}
              if(data.week[i,4]==6 & data.week[i,5]==3){quart.month[i,1]=23}
              if(data.week[i,4]==6 & data.week[i,5]==4){quart.month[i,1]=24}
              if(data.week[i,4]==7 & data.week[i,5]==1){quart.month[i,1]=25}
              if(data.week[i,4]==7 & data.week[i,5]==2){quart.month[i,1]=26}
              if(data.week[i,4]==7 & data.week[i,5]==3){quart.month[i,1]=27}
              if(data.week[i,4]==7 & data.week[i,5]==4){quart.month[i,1]=28}
              if(data.week[i,4]==8 & data.week[i,5]==1){quart.month[i,1]=29}
              if(data.week[i,4]==8 & data.week[i,5]==2){quart.month[i,1]=30}
              if(data.week[i,4]==8 & data.week[i,5]==3){quart.month[i,1]=31}
              if(data.week[i,4]==8 & data.week[i,5]==4){quart.month[i,1]=32}
              if(data.week[i,4]==9 & data.week[i,5]==1){quart.month[i,1]=33}
              if(data.week[i,4]==9 & data.week[i,5]==2){quart.month[i,1]=34}
              if(data.week[i,4]==9 & data.week[i,5]==3){quart.month[i,1]=35}
              if(data.week[i,4]==9 & data.week[i,5]==4){quart.month[i,1]=36}
              if(data.week[i,4]==10 & data.week[i,5]==1){quart.month[i,1]=37}
              if(data.week[i,4]==10 & data.week[i,5]==2){quart.month[i,1]=38}
              if(data.week[i,4]==10 & data.week[i,5]==3){quart.month[i,1]=39}
              if(data.week[i,4]==10 & data.week[i,5]==4){quart.month[i,1]=40}
              if(data.week[i,4]==11 & data.week[i,5]==1){quart.month[i,1]=41}
              if(data.week[i,4]==11 & data.week[i,5]==2){quart.month[i,1]=42}
              if(data.week[i,4]==11 & data.week[i,5]==3){quart.month[i,1]=43}
              if(data.week[i,4]==11 & data.week[i,5]==4){quart.month[i,1]=44}
              if(data.week[i,4]==12 & data.week[i,5]==1){quart.month[i,1]=45}
              if(data.week[i,4]==12 & data.week[i,5]==2){quart.month[i,1]=46}
              if(data.week[i,4]==12 & data.week[i,5]==3){quart.month[i,1]=47}
              if(data.week[i,4]==12 & data.week[i,5]==4){quart.month[i,1]=48}
            }
            ...
            if(TS>1){
              ...
              while (point<=final.point){
                data.at.timescale[c,]=c(data.week[b,3:4],data.week[b,9],colSums(data.week[a:b,6:8]))
                if(is.na(data.at.timescale[c,4])==TRUE || is.na(data.at.timescale[c,5])==TRUE|| is.na(data.at.timescale[c,6]==TRUE)){
            ...
            if (Good=="Yes" || Good=="YES" || Good=="YeS" || Good=="YEs"|| Good=="yes"){
              if (is.numeric(sig.level)==FALSE ||
                  sig.level<0.90 ||  sig.level>0.95){stop("Please provide an appropriate value for the
              ...
              for (i in 1:48){
                ...
                rain=(data.at.timescale[which(data.at.timescale[,3]==month.par),4]);rain.nozero=na.omit(rain[rain>0]);n.rain=length(na.omit(rain))
                n.nonzero=length(rain.nozero); n.z=n.rain-n.nonzero; if(n.z==0){probzero=0}else{probzero=(n.z+1)/(2*(n.rain+1))}
              ...
              for (j in 1:48){
                ...
                if(Norn.check[j,1]<0.960 && Norn.check[j,2]<0.10 && Norn.check[j,3]>0.05){Norn.check[j,10]="NoNorn"}else{Norn.check[j,10]="Normal"}
                if(Norn.check[j,4]<0.960 && Norn.check[j,5]<0.10 && Norn.check[j,6]>0.05){Norn.check[j,11]="NoNorn"}else{Norn.check[j,11]="Normal"}
                if(Norn.check[j,7]<0.960 && Norn.check[j,8]<0.10 && Norn.check[j,9]>0.05){Norn.check[j,12]="NoNorn"}else{Norn.check[j,12]="Normal"}
                ######As proposed in Stagge et al. (2015)
                if(Norn.check[j,2]<0.10){Norn.check[j,13]="NoNorn"}else{Norn.check[j,13]="Normal"}
                if(Norn.check[j,5]<0.10){Norn.check[j,14]="NoNorn"}else{Norn.check[j,14]="Normal"}
                if(Norn.check[j,8]<0.10){Norn.check[j,15]="NoNorn"}else{Norn.check[j,15]="Normal"}
              }
              ##########
            ...
            if (Good=="NO" || Good=="No" || Good=="nO" || Good=="no"){
              for (i in 1:48){
                month.par=data.at.timescale[i,3]
                rain=(data.at.timescale[which(data.at.timescale[,3]==month.par),4]);rain.nozero=na.omit(rain[rain>0]);n.rain=length(rain)
                n.nonzero=length(rain.nozero); n.z=n.rain-n.nonzero; if(n.z==0){probzero=0}else{probzero=(n.z+1)/(2*(n.rain+1))}
                parameters[i,1:4]=c(data.at.timescale[i,3],pelgam(samlmu(rain.nozero)),probzero)
                petp.harg=(data.at.timescale[which(data.at.timescale[,3]==month.par),7])
                petp.pm=(data.at.timescale[which(data.at.timescale[,3]==month.par),8])
    ...
    }else{mesage("Good should be set to either Yes or No.")}
  }else{mesage("distri should be set to either GEV or GLO.")}
  rm(list = ls())}
