    ...
    if (distr == "GEV" || distr == "GLO") {
      ...
      if (plotdata == "Yes" || plotdata == "YES" || plotdata == "YeS" || plotdata == "YEs" || plotdata == "yes" ||
        ...
        if (all.equal(TS, as.integer(TS)) != TRUE || is.na(TS) == TRUE ||
            ...
            for (year in start.year:end.year) {
              ...
              for (month in 1:12) {
                ...
                data.week3 <- colSums(ref[which(ref$YEAR ==
                  year & ref$MM == month & ref$DD > 14 &
                  ref$DD <= 21), 8:9])
                data.week4 <- colSums(ref[
                  which(ref$YEAR ==
                    year & ref$MM == month & ref$DD > 21),
                  8:9
                ])
          ...
          rows <- which(data.week[, 1] == end.year & data.week[
            ,
            2
          ] > end.month)
          n.rows <- length(rows)
          ...
          data.week <- suppressWarnings(cbind(data.week, rep(
            1:48,
            n.years
          )))
          ...
          if (TS > 1) {
            ...
            b <- TS
            c <- 1
            data.at.timescale[c, ] <- c(
              data.week[b, 1:2],
              data.week[b, 6], colSums(data.week[
                a:b,
                4:5
              ])
            )
            ...
            while (point <= end.point) {
              data.at.timescale[c, ] <- c(data.week[
                b,
                1:2
              ], data.week[b, 6], colSums(data.week[
                a:b,
          ...
          colnames(parameters) <- c(
            "lastweek", "alfa.gam",
            "beta.gam", "probzero.rain", "loc.gev", "sc.gev",
            "sh.gev"
          )
          ...
          while (pos <= n.weeks) {
            ...
            prob <- parameters[i, 4] + (1 - parameters[
              i,
              4
            ]) * cdfgam(
              data.at.timescale[pos, 4],
              ...
              if (SDI[i, 1] <= -1.5 & !is.na(SDI[i, 1])) {
                ...
                if (SDI[i, 1] <= -1 & !is.na(SDI[i, 1])) {
                  ...
                  if (SDI[i, 1] <= 1 & !is.na(SDI[i, 1])) {
                    ...
                    if (SDI[i, 1] <= 1.5 & !is.na(SDI[
                      i,
                      1
                    ])) {
                      categories[i, 1] <- "mod.wet"
                    } else {
                      if (SDI[i, 1] <= 2 & !is.na(SDI[
                        i,
                        1
                      ])) {
              ...
              if (SDI[i, 2] <= -1.5 & !is.na(SDI[i, 2])) {
                ...
                if (SDI[i, 2] <= -1 & !is.na(SDI[i, 2])) {
                  ...
                  if (SDI[i, 2] <= 1 & !is.na(SDI[i, 2])) {
                    ...
                    if (SDI[i, 2] <= 1.5 & !is.na(SDI[
                      i,
                      2
                    ])) {
                      categories[i, 2] <- "mod.wet"
                    } else {
                      if (SDI[i, 2] <= 2 & !is.na(SDI[
                        i,
                        2
                      ])) {
